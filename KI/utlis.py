...
class Utlis:
    def textDetection(self,imageName):
        print('Text detection start')
        img = Image.open('cutImg/'+imageName+'_tres.jpg')
        text = tess.image_to_string(img)
        print(text)
    #Bild Bearbeitung
    def testTresh(self):
        print("Test")
        path='cutImg/scan1_calibrate.jpg'
    ...
    def tresholdImg(self,imageName):
        print('Treshold Img')
        path='cutImg/'+imageName+'_calibrate.jpg'
        img = cv2.imread(path)
        image_gamma_correct=self.gamma_trans(1.5,img)
        cv2.imwrite('cutImg/'+imageName+'_gamma.jpg',image_gamma_correct)
        imgGray = cv2.cvtColor(image_gamma_correct, cv2.COLOR_BGR2GRAY)
        imgBlur = cv2.GaussianBlur(imgGray, (5,5),1)
        cv2.imwrite('cutImg/'+imageName+'_bluer.jpg',imgBlur)
        ...
        print(text)
        cv2.imwrite('cutImg/'+imageName+'_tres.jpg',thresh)
